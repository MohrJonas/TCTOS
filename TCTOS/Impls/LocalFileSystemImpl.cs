...
public sealed class LocalFileSystemImpl(ILogger<LocalFileSystemImpl> logger) : IFileSystem
{
    public static class PathHelper
    {
        ...
        public static string GetPerContainerRootPath() 
            => Path.Combine(RootPath, "containers");
        
        public static string GetPerContainerPath(string containerName) 
            => Path.Combine(GetPerContainerRootPath(), containerName);

        public static string GetPerContainerProvisioningFilePath(string containerName)
            => Path.Combine(GetPerContainerPath(containerName), "provision");

        public static string GetPerContainerConfigurationPath(string containerName)
            => Path.Combine(GetPerContainerPath(containerName), "configuration.json");

        public static string GetConfigurationPath()
            => Path.Combine(RootPath, "configuration.json");

        public static string GetFeaturesRootPath()
            => Path.Combine(RootPath, "features");

        public static string GetFeatureRootPath(string featureName)
            => Path.Combine(GetFeaturesRootPath(), featureName);

        public static string GetFeatureDescriptorPath(string featureName)
            => Path.Combine(GetFeatureRootPath(featureName), "descriptor.json");
        
        public static string GetFeatureExecutablePath(string featureName)
            => Path.Combine(GetFeatureRootPath(featureName), "feature");
    }

    public Task<Result<string?>> GetProvisioningFileContentAsync(string containerName) => RunCatchingAsync(
        async () =>
        {
    ...
    public Task<Result> SetProvisioningFileContentAsync(string containerName, string fileContent) => RunCatchingAsync(
        async () =>
        {
    ...
    public Task<Result<ContainerConfiguration?>> GetContainerConfigurationAsync(string containerName) => RunCatchingAsync(
        async () =>
        {
    ...
    public Task<Result> SetContainerConfigurationAsync(string containerName, ContainerConfiguration configuration) => RunCatchingAsync(
        async () =>
        {
    ...
    public Task<Result<TctOsConfiguration?>> GetConfigurationAsync() => RunCatchingAsync(
        async () =>
        {
    ...
    public Task<Result> SetConfigurationAsync(TctOsConfiguration configuration) => RunCatchingAsync(
        async () =>
        {
            logger.LogDebug("Configuration is {configuration}", configuration);
            var path = PathHelper.GetConfigurationPath();
